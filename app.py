...
class Settings(BaseSettings):
    ...
    database_url: str = Field(default="sqlite:///rag_chatbot.db", env="DATABASE_URL")
            ...
            try:
                ...
                if os.path.exists(index_path) and os.path.exists(documents_path):
                    ...
                    if not self.documents or len(self.documents) == 0:
                        logger.error("Documents file is empty or corrupted")
                        raise ValueError("No documents loaded from documents.npy")
                    logger.info(f"Loaded FAISS index from {index_path} with {len(self.documents)} documents")
                else:
                    logger.warning(f"FAISS index ({index_path}) or documents ({documents_path}) not found")
                    raise FileNotFoundError("FAISS index or documents missing")
            except Exception as e:
                logger.error(f"Failed to initialize FAISS or embeddings: {str(e)}. Proceeding without PDF data.")
                self.index = None
                self.documents = []
        except Exception as e:
            logger.error(f"Failed to initialize RAG system: {str(e)}. Using fallback mode.")
            self.groq_client = None
    ...
    def search_knowledge(self, query: str) -> List[Dict]:
        try:
            if self.index and self.embedding_model and self.documents:
                ...
                distances, indices = self.index.search(np.array([embedding]), k=5)
                results = [
                    {"content": self.documents[i]["content"], "similarity": float(distances[0][j])}
                    for j, i in enumerate(indices[0]) if i < len(self.documents)
                ]
                logger.info(f"Retrieved {len(results)} documents for query: {query}")
                return results
            else:
                logger.warning("FAISS index, embedding model, or documents missing")
                return []
        except Exception as e:
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://zeraan.dev",             # Your frontend on Vercel or custom domain
        "https://chatbot-backend.onrender.com",  # Replace with Render URL
